//https://github.com/yliharsilamikko/AUOH-robot-cloud/blob/master/public/mqtt-threejs.js
//check right code form there

let three_view = document.getElementById('three_view'); //hakee documetn
//<div id="three_view"></div> from html
let renderer = new THREE.WebGLRenderer(); //WebGl as OpenGl for native  same effective
let scene = new THREE.Scene();//mailma
...
let near = 0.1; //meters
let far = 1000;
//perspectivi ja orthografu camera
//normali its 0 point
...
//move parts to origin
    ...
    joints[5].geometry.translate(-1.54869, -1.96913, -0.008715);
...
//joints[3].rotation.set(THREE.Math.degToRad(90),0,0);
offsets.push(new THREE.Group());
...
animate(); //first time it start from here and then goues for infinity loop 60 times per sec
...
const mqtt_client = mqtt.connect('wss://mqtt-broker-maria.herokuapp.com/') //check this address
...
mqtt_client.on('message', (topic, message) => {
    ...
    if(joints.length ==6){ //loading firt all info and then start to build
    ...
    joints[3].rotation.set(0, 0, THREE.Math.degToRad(joint_data.joints[2]) - THREE.Math.degToRad(joint_data.joints[1])); //compensation
...
//camera setup  npm run start dev
